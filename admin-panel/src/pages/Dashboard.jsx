        ...
        <Box>
            ...
            <Grid container spacing={3} sx={{ mb: 4 }}>
                ...
                <Grid size={{ xs: 12, sm: 6, md: 3 }}>
                    <StatCard
                        title="Total Revenue"
                        value={`Rs. ${(stats?.stats?.totalRevenue ?? 0).toLocaleString()}`}
                        icon={<DollarSign size={24} />}
                        color="info"
                        subtitle={`Commission: Rs. ${(stats?.stats?.totalCommission ?? 0).toLocaleString()}`}
                    />
                </Grid>
            </Grid>
            ...
            <Grid container spacing={2} sx={{ mb: 4 }}>
                {[
                    { label: 'Review Providers', sub: `${stats?.stats?.pendingApprovals ?? 0} awaiting`, icon: <CheckCircle size={18} />, color: '#FF6B2B', path: '/providers' },
                    { label: 'View Bookings', sub: `${stats?.stats?.totalBookings ?? 0} total`, icon: <CalendarCheck size={18} />, color: '#2196F3', path: '/bookings' },
                    { label: 'Manage Services', sub: 'All service types', icon: <ShoppingBag size={18} />, color: '#4CAF50', path: '/services' },
                    { label: 'Send Notification', sub: 'Broadcast to users', icon: <Bell size={18} />, color: '#9C27B0', path: '/notifications' },
                ].map((a, i) => (
                    <Grid key={i} size={{ xs: 12, sm: 6, md: 3 }}>
                        ...
                        >
                            <Box sx={{ p: 1.5, borderRadius: 2, bgcolor: `${a.color}18`, color: a.color }}>
                                {a.icon}
                            </Box>
                            <Box>
                    ...
                    <Card>
                        <CardContent>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
                                ...
                                <Chip size="small" icon={<TrendingUp size={14} />} label="30 days" variant="outlined" />
                            </Box>
                            <Box sx={{ height: 300 }}>
                                    ...
                                    <TableBody>
                                        {(stats?.recentBookings || []).map((b) => (
                                            <TableRow key={b._id} hover>
                                                <TableCell>
                                                    <Typography variant="body2" fontWeight={600}>{b.orderNumber}</Typography>
                                                </TableCell>
                    ...
                    <Card sx={{ height: '100%' }}>
                        <CardContent>
                            ...
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 2 }}>
                                <Box sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(30, 41, 59, 0.06)', border: '1px solid rgba(30, 41, 59, 0.15)' }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">Waiting for approval</Typography>
                                </Box>
                                <Box sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(142, 45, 226, 0.06)', border: '1px solid rgba(142, 45, 226, 0.15)' }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">Providers currently suspended</Typography>
                                </Box>
                                <Box sx={{ p: 2, borderRadius: 2, bgcolor: 'rgba(76, 175, 80, 0.06)', border: '1px solid rgba(76, 175, 80, 0.15)' }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">Provider earnings to settle</Typography>
                                </Box>
                            </Box>
                        </CardContent>
                    </Card>
