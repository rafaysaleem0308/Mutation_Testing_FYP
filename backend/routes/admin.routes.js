...
const { buildUserResponse, generateAccessToken, generateRefreshToken, REFRESH_TOKEN_EXPIRY_DAYS } = require("./auth.routes");
...
router.get("/dashboard-stats", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        const pendingApprovals = await ServiceProvider.countDocuments({ status: "pending" });
        const activeProviders = await ServiceProvider.countDocuments({ status: "approved" });
        ...
        const totalCommission = revenueData.length > 0 ? revenueData[0].commission : 0;
        ...
        const pendingPayouts = pendingPayoutData.length > 0 ? pendingPayoutData[0].total : 0;
        ...
        const recentBookings = await Order.find()
            .sort({ createdAt: -1 })
            .limit(5)
            .select("orderNumber name providerServiceName totalAmount status createdAt");
...
// ─── PROVIDER MANAGEMENT ──────────────────────────────────────────────────────
router.get("/providers", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const { status, spSubRole } = req.query;
        const query = {};
        if (status) query.status = status;
        if (spSubRole) query.spSubRole = spSubRole;
        const providers = await ServiceProvider.find(query).sort({ createdAt: -1 });
...
router.get("/providers/:id", verifyToken, requireRole("admin"), async (req, res) => {
    try {
...
router.patch("/providers/:id/status", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: `Provider ${status} successfully`, provider });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});

router.delete("/providers/:id", verifyToken, requireRole("admin"), async (req, res) => {
    try {
...
router.patch("/users/:id/status", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.params.id, { accountStatus }, { new: true }).select("-password");
        if (!user) return res.status(404).json({ success: false, message: "User not found" });
        res.status(200).json({ success: true, user });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
...
router.get("/bookings/:id", verifyToken, requireRole("admin"), async (req, res) => {
    try {
...
router.patch("/bookings/:id/cancel", verifyToken, requireRole("admin"), async (req, res) => {
    try {
...
router.patch("/bookings/:id/status", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const { status } = req.body;
        const booking = await Order.findByIdAndUpdate(
            req.params.id,
        ...
        for (const chat of chats) {
            if (chat.participants && chat.participants.length > 0) {
                for (let i = 0; i < chat.participants.length; i++) {
                    ...
                    const userData = await Model.findById(p.user).select("firstName lastName email role profileImage spSubRole").lean();
                    chat.participants[i] = {
                        ...p,
...
router.get("/chats/:chatId/messages", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const messages = await Message.find({ chatId: req.params.chatId }).sort({ createdAt: 1 }).lean();

        // Populate sender info
        for (const msg of messages) {
            let sender = await User.findById(msg.senderId).select("firstName lastName role").lean();
            if (!sender) {
                sender = await ServiceProvider.findById(msg.senderId).select("firstName lastName spSubRole").lean();
            }
...
router.get("/payments", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const payments = await Order.find({ paymentStatus: { $in: ["Completed", "Pending"] } })
            .sort({ createdAt: -1 })
            .select("orderNumber name providerName totalAmount platformCommission providerEarnings paymentMethod paymentStatus createdAt");
...
// ─── NOTIFICATIONS ────────────────────────────────────────────────────────────
router.post("/notifications/send", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        let targetUsers = [];
        if (target === "all") {
            targetUsers = await User.find({ role: { $in: ["user", "service_provider"] } }).select("_id").lean();
        } else if (target === "users") {
            targetUsers = await User.find({ role: "user" }).select("_id").lean();
        } else if (target === "providers") {
            targetUsers = await User.find({ role: "service_provider" }).select("_id").lean();
        ...
        const notificationPromises = targetUsers.map(async (u) => {
            ...
            if (io) {
                ...
                io.to(u._id.toString()).emit("notification", { title, message, type: "admin_broadcast", createdAt: new Date() });
            }
        });
...
router.get("/housing", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        ...
        res.json({ success: true, properties, total, page: parseInt(page), totalPages: Math.ceil(total / parseInt(limit)) });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
});
...
router.get("/housing/stats", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const [total, pending, approved, rejected, suspended, totalBookings, pendingBookings] = await Promise.all([
...
router.get("/housing/:id", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        let property = await HousingProperty.findById(req.params.id)
            .populate("ownerId", "firstName lastName email phone city address profileImage createdAt")
            .lean();

        // Handle legacy hostel fallback
        if (!property) {
            const legacy = await Service.findById(req.params.id).populate("serviceProviderId").lean();
            if (legacy && legacy.serviceType === "Hostel/Flat Accommodation") {
                property = {
                    ...
                    status: legacy.status === "Active" ? "approved" : "pending_approval",
...
router.patch("/housing/:id/status", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const { status, rejectionReason } = req.body;
        const validStatuses = ["approved", "rejected", "suspended", "pending_approval"];
        ...
        let property = await HousingProperty.findByIdAndUpdate(req.params.id, updateData, { new: true })
        ...
        if (!property) {
            ...
            if (legacy) {
                property = { _id: legacy._id, title: legacy.serviceName || legacy.title, status };
            }
        }

        // Notify the owner
        const Notification = require("../models/notification.model");
        const ownerUser = await User.findOne({ spId: property.ownerId._id || property.ownerId });
        if (ownerUser) {
            await Notification.create({
                userId: ownerUser._id,
                title: status === "approved" ? "Property Approved!" : status === "rejected" ? "Property Rejected" : "Property Suspended",
...
router.delete("/housing/:id", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        let property = await HousingProperty.findByIdAndDelete(req.params.id);

        // Handle legacy hostel fallback
        if (!property) {
            property = await Service.findOneAndDelete({ _id: req.params.id, serviceType: "Hostel/Flat Accommodation" });
        }
...
router.get("/housing-bookings", verifyToken, requireRole("admin"), async (req, res) => {
    try {
...
router.patch("/services/:id/status", verifyToken, requireRole("admin"), async (req, res) => {
    try {
        const { status } = req.body;
...
router.delete("/services/:id", verifyToken, requireRole("admin"), async (req, res) => {
    try {
