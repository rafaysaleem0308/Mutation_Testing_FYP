...
async function createPaymentIntent({ bookingId, serviceType, userId }) {
    ...
    if (existing) {
        ...
        return { clientSecret: existing.stripeClientSecret, paymentId: existing._id };
    }
    ...
    if (!provider) {
        ...
        if (serviceDoc && serviceDoc.serviceProviderId) {
            const spDoc = await ServiceProvider.findById(serviceDoc.serviceProviderId);
            if (spDoc && spDoc.email) {
                provider = await User.findOne({ email: spDoc.email });
            }
        }
    ...
    if (!provider) {
        ...
        console.warn("⚠️ Provider User not explicitly found for Provider ID: " + providerId + ". Bypassing strict account status check for Test Mode.");
    } else if (provider.accountStatus && provider.accountStatus !== "active") {
        throw new Error("Provider is not active. Status: " + provider.accountStatus);
    }
...
async function releaseEscrow(paymentId) {
    ...
    const wallet = await getOrCreateWallet(payment.providerId);
    wallet.pendingBalance = Math.max(0, wallet.pendingBalance - payment.providerAmount);
