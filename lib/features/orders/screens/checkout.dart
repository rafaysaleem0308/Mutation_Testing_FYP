...
class CheckoutScreen extends StatefulWidget {
  ...
  const CheckoutScreen({super.key, required this.cartItems, required this.provider});
...
class _CheckoutScreenState extends State<CheckoutScreen> {
  ...
  bool get isLaundry {
    ...
    final type = widget.cartItems[0]['serviceType']?.toString().toLowerCase() ?? '';
  ...
  Future<void> placeOrder() async {
    ...
    if (_deliveryAddressController.text.trim().isEmpty || 
      ...
      _showValidationError("Please fill in all required fields (Address, Phone, and Schedule if applicable).");
      return;
    }
    ...
    try {
      ...
      final providerUserId = widget.provider['userId']?.toString() ?? providerId;

      final orderData = {
        'userId': userId,
        'serviceProviderId': providerUserId.isNotEmpty ? providerUserId : providerId,
        'serviceProviderSpId': providerId,
        'serviceProviderName': widget.provider['username']?.toString() ??
        ...
        'items': widget.cartItems.asMap().entries.map((e) {
          ...
          return {
            ...
            'name': item['name']?.toString() ??
                item['serviceName']?.toString() ?? 'Item',
        ...
        'paymentStatus': _selectedPaymentMethod == 'Cash on Delivery' ? 'Pending' : 'Completed', // Update based on path
      };
      ...
      if (orderResult['success'] != true) {
        ...
        if (mounted) _showValidationError(orderResult['message'] ?? "Failed to create order");
        return;
      }
      ...
      if (_selectedPaymentMethod == 'Cash on Delivery') {
        ...
        if (paymentResult['success'] == true) {
          ...
          _showMsg("Payment cancelled. Your order was created but not paid.", false);
        } else {
          _showValidationError(paymentResult['message'] ?? "Payment failed. Please try again.");
        }
      }
    } catch (e) {
      if (mounted) setState(() => _isLoading = false);
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: Row(children: [
          ...
          Text("Attention", style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
        ]),
        content: Text(msg, style: GoogleFonts.inter()),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx),
            child: Text("OK", style: GoogleFonts.inter(fontWeight: FontWeight.bold, color: const Color(0xFF1E293B))),
          )
        ],
      ),
    );
        ...
        child: Column(
          children: [
            _buildSectionCard("Order Details", [
              ...widget.cartItems.asMap().entries.map((e) => _buildCartItem(e.value, e.key)),
            ...
            _buildSectionCard("Delivery Details", [
              _field("Address", _deliveryAddressController, Icons.location_on_outlined, maxLines: 2),
              const SizedBox(height: 16),
              _field("Phone", phoneController, Icons.phone_outlined,
                  keyboardType: TextInputType.phone),
            ]),
            if (isLaundry) ...[
              const SizedBox(height: 24),
              _buildSectionCard("Schedule (Laundry)", [
                _buildScheduleRow("Pickup", pickupDate, pickupTime,
                    (d) => setState(() => pickupDate = d),
                    (t) => setState(() => pickupTime = t)),
                const Divider(height: 32),
                _buildScheduleRow("Delivery", deliveryDate, deliveryTime,
  ...
  Widget _buildPaymentOption({
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? color.withValues(alpha: 0.05) : const Color(0xFFF9FAFB),
        ...
        child: Row(children: [
          Container(
            ...
            decoration: BoxDecoration(color: isSelected ? color : Colors.grey[200], shape: BoxShape.circle),
            child: Icon(icon, color: isSelected ? Colors.white : Colors.grey[600], size: 22),
          ),
          const SizedBox(width: 14),
          Expanded(
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(title, style: GoogleFonts.poppins(fontWeight: FontWeight.bold, fontSize: 15, color: isSelected ? color : Colors.black87)),
  ...
  Widget _buildCartItem(Map<String, dynamic> item, int index) {
    return Padding(
      ...
      child: Row(children: [
        Expanded(
            child: Text(item['name']?.toString() ?? 'Service',
                style: GoogleFonts.inter(fontWeight: FontWeight.w600))),
        Text("${itemQuantities[index]}x",
  ...
  Widget _buildScheduleRow(String label, DateTime? date, TimeOfDay? time,
      Function(DateTime) onDate, Function(TimeOfDay) onTime) {
      ...
      Row(children: [
        Expanded(
          child: GestureDetector(
            ...
            child: Container(
              ...
              child: Row(children: [
                const Icon(Icons.calendar_today, size: 16, color: Color(0xFF1E293B)),
        ...
        Expanded(
          child: GestureDetector(
            onTap: () async {
              final picked =
                  await showTimePicker(context: context, initialTime: time ?? TimeOfDay.now());
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(12)),
              child: Row(children: [
                const Icon(Icons.access_time, size: 16, color: Color(0xFF1E293B)),
                const SizedBox(width: 8),
      ...
      child: SafeArea(
        child: Row(children: [
          Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text("Total Amount", style: GoogleFonts.inter(fontSize: 12, color: Colors.grey)),
              ...
              child: ElevatedButton(
                ...
                child: _isLoading
                    ...
                    : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        const Icon(Icons.lock_clock_rounded, size: 18, color: Colors.white),
                        const SizedBox(width: 8),
                        Text(_selectedPaymentMethod == 'Cash on Delivery' ? "Confirm Order (COD)" : "Pay with Card",
                            style: GoogleFonts.poppins(
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            Text(
                isCOD ? "Cash on Delivery confirmed" : "PKR ${amount.toStringAsFixed(2)} paid via Stripe",
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text("Back to Home",
                    style: GoogleFonts.poppins(
                        fontWeight: FontWeight.bold, color: const Color(0xFF635BFF))),
              ),
...
class OrderConfirmationScreen extends StatelessWidget {
  final Map<String, dynamic> order;
  final Map<String, dynamic> provider;
  const OrderConfirmationScreen({super.key, required this.order, required this.provider});
