...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _saveProfile() async {
    ...
    try {
      ...
      final userId =
          userData?['id'] ?? userData?['userId'] ?? userData?['_id'] ?? '';
      if (userId.isEmpty) {
        ...
        if (imageResult['success'] == true) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  imageResult['message'] ?? "Failed to upload image",
                ),
                backgroundColor: Colors.red,
              ),
            );
                ...
                child: Column(
                  children: [
                    ...
                    if (userData?['role'] == 'user')
                      _buildInfoSection("Family Contact", [
                        _infoTile(
                          Icons.people_outline,
                          "Contact Name",
                          _familyContractController,
                          enabled: _isEditing,
                        ),
                        _infoTile(
                          Icons.call_outlined,
                          "Contact Phone",
                          _familyPhoneController,
                          enabled: _isEditing,
                          keyboardType: TextInputType.phone,
                        ),
                      ]),
    ...
    return Container(
      ...
      child: Column(
        children: [
              ...
              children: [
                CircleAvatar(
                  ...
                  child: imageProvider == null
                      ? Icon(Icons.person, size: 50, color: Color(0xFF1E293B))
                      : null,
                ),
                if (_isEditing)
                  Positioned(
          ...
          Text(
            userData?['role']?.toString().toUpperCase() ?? "MEMBER",
            style: GoogleFonts.inter(
              fontSize: 12,
              fontWeight: FontWeight.bold,
