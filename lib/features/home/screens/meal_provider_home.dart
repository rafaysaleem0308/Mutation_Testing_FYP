...
class _MealProviderHomeState extends State<MealProviderHome>
  ...
  Widget _buildDashboard() {
    ...
    final history = _orders
        .where((o) =>
            ['Delivered', 'Completed', 'Cancelled', 'Rejected'].contains(o['status']))
        .toList();
    ...
    return Padding(
      ...
      child: Row(children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text("Manage your kitchen & orders",
        ...
        IconButton(
            icon: const Icon(Icons.forum_outlined, color: Color(0xFFFF9D42)),
            onPressed: () => Navigator.pushNamed(context, '/my-chats')),
        IconButton(
            icon:
                const Icon(Icons.notifications_outlined, color: Color(0xFFFF9D42)),
  ...
  Widget _buildQuickStats() {
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
            child: _statCard("Pending", "${_stats['pendingOrders'] ?? 0}",
                Icons.pending_actions, Colors.orange)),
        const SizedBox(width: 12),
        Expanded(
            child: _statCard("Total", "${_stats['totalOrders'] ?? 0}",
                Icons.shopping_bag_outlined, Colors.indigo)),
      ]),
    );
  }
  ...
  Widget _buildOrderList(List<Map<String, dynamic>> orders,
      {bool isRequest = false, bool isActive = false}) {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Container(
              ...
              decoration: BoxDecoration(
                  color: sCol.withOpacity(0.1), shape: BoxShape.circle),
          ...
          if (o['customerId'] != null)
            IconButton(
                icon: Icon(Icons.chat_bubble_outline_rounded,
                    color: const Color(0xFFFF9D42), size: 20),
                onPressed: () =>
        ...
        _detailRow(
            Icons.person_outline, "Customer", o['name'] ?? 'Guest'),
        ...
        ...(o['items'] as List? ?? []).map((item) => Padding(
              ...
              child: Row(children: [
                ...
                Expanded(
                    child: Text(item['serviceName'] ?? 'Item',
                        style: GoogleFonts.inter(fontSize: 13))),
                Text("PKR ${item['totalPrice'] ?? 0}",
                    style: GoogleFonts.inter(
  ...
  Widget _buildStatusButton(Map<String, dynamic> o) {
    final s = o['status'];
    if (s == 'Confirmed') {
      return _fullBtn("Start Cooking", Colors.blue,
          () => _updateStatus(o['_id'], 'Preparing'));
    }
    if (s == 'Preparing') {
      return _fullBtn("Ready for Pickup", Colors.orange,
          () => _updateStatus(o['_id'], 'Ready for Delivery'));
    }
    if (s == 'Ready for Delivery') {
      return _fullBtn("Out for Delivery", Colors.purple,
          () => _updateStatus(o['_id'], 'Out for Delivery'));
    }
    if (s == 'Out for Delivery') {
      return _fullBtn("Mark Delivered", Colors.green,
  ...
  Widget _buildServices() {
    return Scaffold(
      ...
      appBar: AppBar(
          ...
          actions: [
            IconButton(
                icon: const Icon(Icons.add_circle_outline, color: Color(0xFFFF9D42)),
                tooltip: "Add Dish",
                onPressed: () async {
                      ...
                      child: Row(children: [
                        if (!hasImage)
                          Container(
                            width: 52, height: 52,
                            decoration: BoxDecoration(
                                color: const Color(0xFFFF9D42).withOpacity(0.12),
                                borderRadius: BorderRadius.circular(14)),
                            child: const Icon(Icons.lunch_dining, color: Color(0xFFFF512F)),
                          ),
                        if (!hasImage) const SizedBox(width: 12),
                        Expanded(
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(s['serviceName'] ?? 'Item',
                        ...
                        Expanded(
                          child: OutlinedButton.icon(
                            ...
                            style: OutlinedButton.styleFrom(
                              ...
                              side: const BorderSide(color: Color(0xFFFF9D42)),
                              shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
                            ),
                            onPressed: () async {
                              final result = await Navigator.push(
                                context,
                                MaterialPageRoute(builder: (_) => AddMealServiceForm(existingService: s)),
                              );
                              if (result == true) _loadServices();
                            },
                          ),
  ...
  Future<void> _confirmDelete(String serviceId) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        title: Text("Delete Dish?", style: GoogleFonts.poppins(fontWeight: FontWeight.bold)),
      ...
      if (mounted) {
        if (result['success'] == true) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text("Dish deleted"), backgroundColor: Colors.red, behavior: SnackBarBehavior.floating));
        } else {
  ...
  Widget _buildAnalytics() {
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text("Track your performance & earnings",
            style: GoogleFonts.inter(fontSize: 13, color: Colors.grey)),
        const SizedBox(height: 20),
        _largeStatCard("Total Earnings",
            "PKR ${_stats['totalEarnings'] ?? 0}", Icons.payments, Colors.green),
        const SizedBox(height: 12),
        Row(children: [
          Expanded(
              child: _largeStatCard("Today",
                  "PKR ${_stats['todayEarnings'] ?? 0}", Icons.trending_up, Colors.blue)),
          const SizedBox(width: 12),
          Expanded(
              child: _largeStatCard("Orders",
                  "${_stats['totalOrders'] ?? 0}", Icons.shopping_bag, Colors.purple)),
        ]),
        ...
        _perfRow(
            ...
            Icons.star_outline,
            Colors.amber),
        _perfRow("Total Orders", "${_stats['totalOrders'] ?? 0}",
            Icons.shopping_bag_outlined, Colors.indigo),
        _perfRow("Cancelled", "${_stats['cancelledOrders'] ?? 0}",
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Container(
          ...
          child:
            ...
            const SizedBox(height: 8),
            Text("PKR ${_stats['totalEarnings'] ?? 0}",
                style: GoogleFonts.poppins(
                    fontSize: 36,
                    fontWeight: FontWeight.bold,
                    color: Colors.white)),
        ...
        const SizedBox(height: 12),
        _perfRow("Today's Earnings", "PKR ${_stats['todayEarnings'] ?? 0}",
            Icons.today, Colors.green),
        _perfRow("Pending Amount", "PKR ${_stats['pendingEarnings'] ?? 0}",
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        padding: EdgeInsets.symmetric(
            horizontal: sel ? 16 : 12, vertical: 10),
        decoration: BoxDecoration(
