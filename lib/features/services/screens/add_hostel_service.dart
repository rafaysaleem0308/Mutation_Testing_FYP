...
class _AddHostelServiceFormState extends State<AddHostelServiceForm> {
  ...
  final _features = ['WiFi', 'Geyser', 'AC', 'Generator', 'Security Guard', 'CCTV', 'Parking', 'Laundry'];
  ...
  @override
  void dispose() {
    _nameCtrl.dispose(); _addressCtrl.dispose(); _priceCtrl.dispose();
    _descCtrl.dispose(); _contactCtrl.dispose();
    super.dispose();
  }
      ...
      if (mounted) {
        ...
        if (result['success'] == true) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
            content: Text(_isEditing ? "Listing updated!" : "Listing published!"),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(_isEditing ? "Edit Accommodation" : "List Accommodation",
        ...
        child: Form(key: _formKey, child: Column(children: [
          ...
          _section("Property Details", [
            ...
            DropdownButtonFormField<String>(
              value: _types.contains(_accommodationType) ? _accommodationType : _types.first,
              decoration: InputDecoration(
                prefixIcon: const Icon(Icons.category_outlined, color: _color, size: 20),
              ...
              items: _types.map((t) => DropdownMenuItem(value: t, child: Text(t, style: GoogleFonts.inter()))).toList(),
              onChanged: (v) => setState(() => _accommodationType = v!),
            ),
            const SizedBox(height: 14),
            _field("Monthly Rent (PKR) *", _priceCtrl, Icons.payments_outlined, keyboardType: TextInputType.number),
          ]),
          const SizedBox(height: 20),

          _section("Location & Contact", [
            _field("Full Address *", _addressCtrl, Icons.location_on_outlined, maxLines: 2),
            const SizedBox(height: 14),
            _field("Contact Number *", _contactCtrl, Icons.phone_outlined, keyboardType: TextInputType.phone),
            if (_currentPosition != null)
              Padding(
                ...
                child: Row(children: [
                  ...
                  Text("GPS location captured ✓", style: GoogleFonts.inter(fontSize: 11, color: Colors.green)),
                ]),
              ),
          ]),
          const SizedBox(height: 20),
          ...
          _section("Capacity", [
            _counterRow("Available Rooms", _availableRooms, (v) => setState(() => _availableRooms = v)),
            const Divider(height: 20),
            Container(
              decoration: BoxDecoration(color: const Color(0xFFF8FAFC), borderRadius: BorderRadius.circular(14)),
              child: SwitchListTile(
                title: Text("Shared Property", style: GoogleFonts.inter(fontWeight: FontWeight.w600)),
                subtitle: Text("Multiple tenants in one unit", style: GoogleFonts.inter(fontSize: 12, color: Colors.grey)),
                value: _isShared, activeColor: _color,
                onChanged: (v) => setState(() => _isShared = v),
              ),
            ),
            if (_isShared) ...[
              const SizedBox(height: 12),
              _counterRow("Max Occupants", _maxOccupants, (v) => setState(() => _maxOccupants = v)),
              const SizedBox(height: 8),
              _counterRow("Current Occupants", _currentOccupants, (v) => setState(() => _currentOccupants = v)),
            ],
          ]),
          const SizedBox(height: 20),

          _section("Amenities & Features", [
            Text("Select all available:", style: GoogleFonts.inter(fontSize: 13, color: Colors.grey[600])),
            const SizedBox(height: 12),
            Wrap(
              ...
              children: _features.map((f) {
                ...
                return FilterChip(
                  ...
                  selected: sel, selectedColor: _color,
          ...
          _section("Description", [
            _field("Facilities, Rules & Notes *", _descCtrl, Icons.description_outlined, maxLines: 4),
          ]),
      ...
      child: Container(
        ...
        child: preview == null
            ? Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                const SizedBox(height: 8),
                Text("Add Property Photo (Optional)", style: GoogleFonts.inter(color: Colors.grey[600])),
              ])
      ...
      children: [
        ...
        Row(children: [
          ...
          Padding(
            ...
            child: Text('$value', style: GoogleFonts.poppins(fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          Container(
            decoration: BoxDecoration(color: _color.withValues(alpha: 0.1), shape: BoxShape.circle),
            child: IconButton(icon: const Icon(Icons.add, color: _color, size: 18), onPressed: () => onChanged(value + 1)),
          ),
        ]),
      ],
    );
